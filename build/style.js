...
cpFile(PATH_TO_FILE, path.join(__dirname, '../dist/style.css'))
...
postcss([
  ...
  .process(fs.readFileSync(PATH_TO_FILE))
  .then(result => {
    fs.writeFile(PATH_TO_SIMPLIFIED_FILE, result.css, () => true)
  })
...
postcss([cssnano()])
  ...
  .then(result => {
    fs.writeFile(PATH_TO_SIMPLIFIED_MINIFIED_FILE, result.css, () => true)
  })
